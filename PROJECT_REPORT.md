...
Two custom, MITRE ATT&CK-mapped detection rules were built and confirmed working through live testing: one detecting unauthorized modification of /etc/passwd (persistence via account manipulation), and one detecting tampering with /etc/sudoers (privilege escalation). A further two rules were written for SSH key-based persistence and credential-file tampering. An attempted third detection approach - real-time command-execution monitoring - hit a genuine platform constraint (Docker on Apple Silicon lacking kernel audit access) and is documented honestly as a limitation rather than concealed.
